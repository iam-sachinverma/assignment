...
const Variables = ({ variables, setVariables }) => {
  ...
  return (
    <div className="p-4 h-full overflow-y-auto">
      <h1 className="text-dark">My variables</h1>
      {variables && (
        <div className="py-4 flex flex-col gap-2">
          {Object.keys(variables).map((varName, i) => (
            <VariableInp
              key={i}
              name={varName}
              val={variables[varName]}
              toggleTruth={() => toggleTruth(varName)}
              onNameChanged={changeVarName}
              deleteVariable={() => deleteVariable(varName)}
            />
          ))}
        </div>
      )}
